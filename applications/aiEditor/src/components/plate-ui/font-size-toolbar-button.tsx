...
export function FontSizeToolbarButton() {
  ...
  return (
    <div className="flex h-7 items-center gap-0 rounded-md bg-muted/60 p-0">
      <ToolbarButton className="text-gray-500 hover:text-gray-900" onClick={() => handleFontSizeChange(-1)}>
        <Minus size={10}/>
      </ToolbarButton>
      ...
      <Popover open={isFocused} modal={false}>
        <PopoverTrigger asChild>
          <input
            className={cn(
              'h-full w-8 shrink-0 bg-transparent px-1 text-center text-[10px] text-gray-500 hover:bg-muted'
            )}
            value={displayValue}
            onBlur={() => {
              setIsFocused(false);
